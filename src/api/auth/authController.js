...
class authController {
    async register(req, res) {
        try {
            ...
            if (check_user.completed && !check_user.result) {
                const { result } = await authModel.registerUser({ username, password: password_encrypted, first_name, last_name, email })
    ...
    async login(req, res) {
        ...
        try {
            ...
            if (check_user.completed && !!check_user.user) {
                const compare_password = await decrypted(check_user.user.password, password)
                if (!compare_password) {
                    return failed(res, 'username หรือ password ไม่ถูกต้อง')
                }
                generateToken(req, check_user.user._id)
                return success(res, "เข้าสู่ระบบเรียบร้อย", { user: check_user.user, token_id: req.token })
            } else {
                return failed(res, 'username หรือ password ไม่ถูกต้อง')
            }
        } catch (error) {
